...
# В --windowed сборке PyInstaller sys.stdout/stderr = None -> любая попытка
# что-то напечатать (в т.ч. внутренний обработчик ошибок Tkinter) падает
# с AttributeError и может "подвесить" обработку событий. Подставляем заглушки.
...
class App(ctk.CTk):
    ...
    def log(self, msg):
        # Единая точка логирования — видна в самом окне приложения,
        # не зависит от наличия консоли у exe
    ...
    def connect(self):
        ...
        # Открытие порта + пауза на перезагрузку ESP32 — в фоновом потоке,
        # иначе GUI виснет на эти 2 секунды (выглядит как "не отвечает")
    ...
    def _poll_queue(self):
        ...
        # ВАЖНО: self.after(...) в конце должен выполниться всегда,
        # иначе при любой непойманной ошибке цикл обновления умирает молча.
        try:
            while True:
                try:
    ...
    def _handle_line(self, line):
        ...
        self.label_throttle.configure(
            text=f"Газ: {obj.get('throttle_pct', 0)}% (PWM: {obj.get('pwm', '—')} мкс)"
        )

...
Отличия от присланной тобой версии (исправленные баги):
  1. Обновление GUI и графика ИДЁТ ТОЛЬКО из главного потока — чтение Serial
     ...
     а GUI разбирает очередь через self.after() каждые 50 мс.
  ...
  5. Датчик тока/напряжения — INA219 (I2C), поля теперь "voltage" (В) и
     "current_a" (А) — это уже РЕАЛЬНЫЕ единицы (а не сырой АЦП, как было
     раньше с voltage_raw/current_raw).

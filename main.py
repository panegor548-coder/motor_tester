...
5. Кнопка "Обновить" — полный перезапуск приложения с нуля (os.execl).
6. Разгон газа теперь идёт по прогрессивной (экспоненциальной) кривой:
   мелкий частый шаг у порога трогания мотора, крупный шаг ближе к максимуму.
"""
...
# Параметры прогрессивной (экспоненциальной) кривой разгона газа:
# в начале диапазона (у порога трогания мотора с места) шаг между
# соседними значениями PWM маленький и частый — это самая рискованная
# зона по броску тока. Ближе к максимуму шаг крупнее, чтобы весь тест
# не растягивался на минуты при том же интервале между командами.
RAMP_POINTS = 120          # сколько промежуточных значений PWM от 1000 до 2000
RAMP_CURVE_STEEPNESS = 3.0  # k: больше = ещё более плавный старт и резче финиш
...
class App(ctk.CTk):
    ...
    def _generate_exp_ramp(self, start=1000, end=2000, points=RAMP_POINTS, k=RAMP_CURVE_STEEPNESS):
        """Возвращает список целых значений PWM от start до end по
        экспоненциальной (ease-in) кривой: у начала соседние значения
        близки друг к другу (медленный, мелкий шаг), к концу — расстояние
        между соседними точками растёт (шаг крупнее, разгон быстрее)."""
        curve = []
        span = end - start
        for i in range(points + 1):
            frac = i / points
            # Ease-in: (e^(k*frac) - 1) / (e^k - 1), растёт от 0 до 1 нелинейно
            shaped = (math.exp(k * frac) - 1) / (math.exp(k) - 1)
            curve.append(int(round(start + shaped * span)))
        curve[-1] = end  # гарантируем, что кривая точно доходит до максимума
        return curve
    ...
    def start_test(self):
        ...
        self.log(f"Запуск теста. Армирование {self.arm_delay:.1f} с на минимальном газу, "
                 f"затем прогрессивный (экспоненциальный) разгон, {RAMP_POINTS} точек, "
                 f"интервал между точками {int(self.ramp_delay * 1000)} мс")
    ...
    def _smooth_ramp_worker(self):
        ...
        # 2) Прогрессивный (экспоненциальный) разгон: у начала диапазона
        #    точки кривой расположены часто и близко друг к другу (мелкий,
        #    осторожный шаг у порога трогания мотора), к максимуму —
        #    реже и с большим шагом (разгон быстрее). Задержка между
        #    точками по-прежнему берётся с ползунка.
        ramp_curve = self._generate_exp_ramp(start=1000, end=2000)
        for target_pwm in ramp_curve:
            if not self.is_running:
                break
    ...
    def _restart_app(self):
        ...
        # ВАЖНО: os.execl() здесь использовать нельзя. Он подменяет образ
        # ТЕКУЩЕГО процесса "на месте" — а в собранном PyInstaller onefile
        # .exe это ломает уже загруженные C-расширения (matplotlib/ft2font
        # и т.п.): они остаются в памяти в промежуточном состоянии, и
        # повторная инициализация Python видит их как "наполовину
        # импортированный модуль" -> ImportError circular import.
        # Правильный способ — запустить НОВУЮ копию процесса и завершить
        # текущий, а не подменять его изнутри.
        try:
            if getattr(sys, "frozen", False):
                # Собранный PyInstaller .exe: sys.executable указывает
                # на сам .exe, дополнительный интерпретатор не нужен.
                subprocess.Popen([sys.executable] + sys.argv[1:])
            else:
                # Запуск как обычный .py-скрипт через python.exe.
                subprocess.Popen([sys.executable] + sys.argv)
        except Exception as e:
            # Если новый процесс не запустился — не убиваем текущий молча,
            # иначе пользователь останется вообще без открытого приложения.
            self.log(f"Не удалось перезапустить приложение: {e}")
            return
 
        # os._exit() вместо sys.exit(): гарантированно и немедленно
        # завершает процесс без прогона atexit/деструкторов Tkinter и
        # matplotlib, которые в момент перезапуска уже частично закрыты
        # (self.destroy() выше) и могут падать с побочными ошибками.
        os._exit(0)
    ...
    def _handle_line(self, line):
        ...
        # Порог снижен с 10 до 2: теперь шагов разгона на порядок меньше
        # (RAMP_POINTS вместо шага 1 мкс на весь диапазон), поэтому старый
        # порог оставлял бы на графике всего ~12 точек на весь тест.
        if self.step_counter >= 2 or not self.is_running:
            for k in self.data:
                if k in obj:
